...
void robustanalyzer::analyzersinglefile(int splitCnt) {
  ...
  addhist("mettrig_dipho10gt2_veto_el");
  addhist("reftrig_mettrig_dipho10gt2_veto_el");
  ...
  while(tree->Next()) {
	...
	//else veto_eeel_idx.push_back(idx);
      ...
      midbarsel *= (*elhoe)->at(idx) < (0.0241+1.28/energy+0.0422*(*(*rho))/energy);
    ...
    if(mettrigs && veto_el_idx.size()>=2) evtcnt_mettrig_dipho10_veto++;
